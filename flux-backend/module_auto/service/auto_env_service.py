...
class AutoEnvService:
    ...
    @classmethod
    async def get_auto_env_list(cls, query_db: AsyncSession, query_object: AutoEnvPageModel, data_scope_sql: str) -> [list | PageResponseModel]:
    ...
    @classmethod
    async def export_auto_env_list(cls, query_db: AsyncSession, query_object: AutoEnvPageModel, data_scope_sql) -> bytes:
        ...
        new_data = [
            {mapping_dict.get(key): value for key, value in item.items() if mapping_dict.get(key)} for item in auto_env_list
        ]
