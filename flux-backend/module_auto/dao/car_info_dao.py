...
from module_auto.entity.do.car_info_do import CarInfo
...
class CarInfoDao:
    ...
    async def get_car_info_list(cls, db: AsyncSession,
        ...
        query = (
            select(CarInfo)
            .where(
                CarInfo.car_name == query_object.car_name if query_object.car_name else True,
                ...
                CarInfo.lat == query_object.lat if query_object.lat else True,
                CarInfo.lng == query_object.lng if query_object.lng else True,
                CarInfo.location == query_object.location if query_object.location else True,
                CarInfo.manager == query_object.manager if query_object.manager else True,
                CarInfo.price == query_object.price if query_object.price else True,
                CarInfo.del_flag == '0',
